...
MongoDB 与 ElasticSearch 的一些对比以及技术选型的建议
...
| 是否支持分片   | 是                   | 是                 |                                                              |
| 是否支持副本   | 是                   | 是                 |                                                              |
| 选主算法       | Bully          | Bully        | 相比于Paxos和Raft算法实现更简单并有一定可靠性上的妥协，但是选举速度比较快 |
...
现状：早期系统的业务日志统一存储在 ES 中，由于数据结构设计不统一、字段冗余，日志存储效率较低，在磁盘资源有限的情况下，实际只能保留一到两个月的数据，对问题回溯支持非常有限。

在分析日志使用场景后发现，这部分业务日志以按条件查询、问题排查为主，对全文检索和复杂聚合依赖不高。基于此，对继续使用 ES 和 切换至 MongoDB 的成本与收益进行了评估，综合存储效率、查询方式和改造成本，最终选择将该类日志切换至 MongoDB 存储。

|          | ElasticSearch                                                | Mongodb                    | 比较                                         |
| -------- | ------------------------------------------------------------ | -------------------------- | -------------------------------------------- |
| 插入速度 | ES需要写入多种数据结构，采用日志先写模式                     | 只写入文档                 | Mongodb写入较快                              |
| 查询速度 | 全文检索查询较快                                             | 根据索引查询               | Mongodb索引查询与ES DocValue结构查询速度类似 |
| 存储成本 | _source需要保存一份完整数据、DocValue保存索引数据、倒排索引需要分词存储数据 | 文档采用压缩算法压缩比较好 | Mongodb整体上存储成本更低                    |

切换完成后，在相同磁盘资源下，日志可保留周期从原来的 约 2 个月提升到至少 6 个月（理论上可支撑 8 个月）。在不额外增加硬件成本的前提下，显著增强了问题排查的可追溯性，避免因日志缺失导致的问题定位困难，使问题分析更加有依据。

![](https://raw.githubusercontent.com/du-mozzie/PicGo/master/images/20260216161300207.png)

![](https://raw.githubusercontent.com/du-mozzie/PicGo/master/images/20260216161332772.png)

## 总结

MongoDB和Elasticsearch都是不错的存储产品

两者的功能特性也存在很多重合的地方, 其实现在很多数据库产品都在互相借鉴, 功能和特性都在逐渐变得相似, 这也是未来很多存储产品的发展趋势, 大家都希望自己能覆盖尽量多的场景和用户群体
